...
def home(request):
    if request.method == 'POST':
        categoria = request.POST.get('dato')
        productos = Producto.objects.filter(tipo__nombre = categoria)
        lista_productos = FiltroProductos(None, queryset=productos)
        return render(request, 'detalle.html', {'lista_productos':lista_productos})
    else:
